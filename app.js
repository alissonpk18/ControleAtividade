(() => {
    ...
    function renderHtmlCards(aoa, containerId) {
        ...
        for (let i = 1; i < aoa.length; i++) {
            ...
            actionContainer.append(btnTimerToggle, btnTimerFinalize, btnConcluir, btnCardEdit);
    ...
    function salvarEdicaoModal() {
        ...
        const statusCanonico = statusEhConcluida(valStatus) ? "Concluída" : valStatus;
    ...
    function iniciarCronometro(rowIndex) {
        ...
        const statusAtual = String(dadosProcessadosGlobais[rowIndex][COLUMNS.status] || "").trim();
        if (!statusAtual) {
    ...
    function sincronizarEstadoCartao(rowIndex) {
        ...
        if (btnConcluir) btnConcluir.style.display = horaFimValor.trim() ? "inline-block" : "none";
    }
